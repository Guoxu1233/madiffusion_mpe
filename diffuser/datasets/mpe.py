...
import numpy as np
import torch
from ddpg_agent import DDPGAgent#看ddpg_agent这个文件夹的init文件，使得ddpg_agent被识别成了一个包
...
'''
prey是被捕食的，predators是捕食的
这里相当于给被捕食的一个预训练权重，并且prey用的就是传统的ddpg算法
'''
...
def sequence_dataset(env, preprocess_fn, seed: int = 9):##################TODO 本来seed=None
    ...
    for idx, seed_dir in enumerate(seed_dirs):
        ...
        #print('this is act', actions.shape) this is act (1000000, 3, 2)
        rewards = np.stack(
            [
        ...
        #print('this is rew', rewards.shape) this is rew (1000000, 3)
        dones = np.stack(
            [
        ...
        #print('this is dones', dones[0].shape) #this is dones (1000000, 3)
        ...
        for obs, act, rew, done in zip(observations, actions, rewards, dones):
            ...
            #print('this is data______________',data_)#这里面每一次的每一个key都对应这16个值
            #print('this is maxtimestep',env.max_timestep)#最大时间步数是25
